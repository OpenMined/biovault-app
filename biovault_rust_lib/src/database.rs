...
pub fn create_genome_database(
    ...
    let db_name = format!("{}_{}", custom_name.replace(' ', "_"), 
...
/// Get app's documents directory for SQLite files
pub fn get_documents_dir() -> Result<std::path::PathBuf, Box<dyn Error>> {
    // This will be called from the Expo module with the proper documents path
