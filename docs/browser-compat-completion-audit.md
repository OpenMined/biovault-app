...
- The live `main` GitHub Actions workflow currently exposes only the legacy
  `deploy_ref` manual input. The provider/completion inputs in this workspace
  must be committed and pushed to a branch, or merged to `main`, before a CI
  provider run can be dispatched with `compat_web_url`, `compat_include_ios`,
  `compat_remote_targets`, or `compat_completion`. Check the selected ref with
